...
def create_net(X, upsample_method='deconv'):
    ...
    with tf.compat.v1.variable_scope('initconv_0'):
        h = relu(inst_norm(conv2d(h, 3, 16, 9, [1, 1, 1, 1])))
    with tf.compat.v1.variable_scope('initconv_1'):
        h = relu(inst_norm(conv2d(h, 16, 32, 3, [1, 2, 2, 1])))
    with tf.compat.v1.variable_scope('initconv_2'):
        h = relu(inst_norm(conv2d(h, 32, 64, 3, [1, 2, 2, 1])))

    # Residual layers
    with tf.compat.v1.variable_scope('resblock_0'):
        h = res_layer(h, 64, 3, [1, 1, 1, 1])
    with tf.compat.v1.variable_scope('resblock_1'):
        h = res_layer(h, 64, 3, [1, 1, 1, 1])
    with tf.compat.v1.variable_scope('resblock_2'):
        h = res_layer(h, 64, 3, [1, 1, 1, 1])
    with tf.compat.v1.variable_scope('resblock_3'):
        h = res_layer(h, 64, 3, [1, 1, 1, 1])
    with tf.compat.v1.variable_scope('resblock_4'):
        h = res_layer(h, 64, 3, [1, 1, 1, 1])

    # Upsampling layers (tanh on last to get 0,255 range)
    if upsample_method is 'deconv':
        with tf.compat.v1.variable_scope('upsample_0'):
            h = relu(inst_norm(deconv2d(h, 64, 32, 3, [1, 2, 2, 1])))
        with tf.compat.v1.variable_scope('upsample_1'):
            h = relu(inst_norm(deconv2d(h, 32, 16, 3, [1, 2, 2, 1])))
        with tf.compat.v1.variable_scope('upsample_2'):
        ...
        with tf.compat.v1.variable_scope('upsample_2'):  # Not actually an upsample.
...
def conv2d(X, n_ch_in, n_ch_out, kernel_size, strides, name=None,
    ...
    W = tf.compat.v1.get_variable(name=name,
                        shape=shape,
                        dtype=tf.float32,
...
def upconv2d(X, n_ch_in, n_ch_out, kernel_size, strides):
    ...
    upsized = tf.image.resize(X, [new_h, new_w]) # method=1)
